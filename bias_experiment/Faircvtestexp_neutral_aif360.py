...
scores        = np.ravel(fairCVtestNeutral['Scores'])   # ce script: scénario neutral
scoresGender  = np.ravel(fairCVtestGender['Scores'])    # non utilisé ici)
scoresEthnic  = np.ravel(fairCVtestEthnic['Scores'])    # non utilisé ici)
gender_labels = profiles_test[:, 1].astype(int)         # 0=Homme (priv), 1=Femme (non-priv)
...
for seed in range(1, 11):
    ...
    score_threshold = np.percentile(scores, 100*(1 - labels.mean())) #binariser score en gardant en gardant meme proportion bons que lables 
    scores_bin = (scores >= score_threshold).astype(int)

    df = pd.DataFrame(profiles_test)
    df['label'] = labels.ravel()
    df['gender'] = gender_labels.ravel()
    ...
    dataset_pred.scores = scores.reshape(-1, 1) #ajout des scores originaux continus
...
df_results.to_csv(OUT_DIR / "FairCV_neutral_bias_metrics_10runs.csv", index=False)
print(f"\nRésultats exportés : {OUT_DIR / 'FairCV_neutral_bias_metrics_10runs.csv'}")