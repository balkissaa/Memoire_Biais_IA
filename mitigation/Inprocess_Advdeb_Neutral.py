...
df_train = pd.DataFrame(profiles_train)
...
clf.fit(profiles_train, y_train)
baseline_preds = clf.predict(profiles_test)

df_baseline_pred = df_test.copy()
df_baseline_pred['label'] = baseline_preds
dataset_baseline_pred = BinaryLabelDataset(
    df=df_baseline_pred, label_names=['label'], protected_attribute_names=['gender']
)
...
for seed in range(1, 11):
    ...
    print("\n  Fairness Metrics APRÈS Adversarial Debiasing  ")
...
out_csv = DATA_PATH / "FairCV_inprocessinggender_metrics_10seeds_neutral.csv"
